import React from 'react';
import { useLocation } from 'react-router-dom';

const About = () => {
  const location = useLocation();

  return (
    <section className="bg-slate-900 text-white px-6 md:px-20 py-20 space-y-20">
      {/* Hero */}
      <div className="text-center space-y-6">
        <h1 className="text-4xl md:text-5xl font-extrabold text-blue-400 mt-12 md:mt-20 tracking-tight leading-tight">
          We build the tech you imagine.
        </h1>
        <p className="text-lg md:text-xl max-w-3xl mx-auto text-slate-300 leading-relaxed">
          TechMate is your dedicated partner for bringing digital ideas to life—from custom websites and web apps to mobile and desktop software.
        </p>
      </div>

      <hr className="border-slate-700" />

      {/* Mission */}
      <div className="max-w-4xl mx-auto text-center space-y-4">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 tracking-tight">Our Mission</h2>
        <p className="text-slate-300 leading-relaxed">
          At TechMate, we believe tech should empower ideas, not complicate them.<br />
          We started this venture to help individuals, startups, and businesses turn their visions into real, usable, launch-ready products—without the headache.
        </p>
      </div>

      <hr className="border-slate-700" />

      {/* What We Build */}
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-white text-center mb-6 tracking-tight">What We Build</h2>
        <ul className="grid md:grid-cols-2 gap-4 text-slate-300 text-base md:text-lg list-disc list-inside leading-relaxed">
          <li>Websites & Web Applications</li>
          <li>iOS & Android Mobile Apps (Native or Cross-Platform)</li>
          <li>Desktop Software (macOS, Windows, Linux)</li>
          <li>MVPs & Prototypes</li>
          <li>Custom APIs & Backend Systems</li>
          <li>Ongoing Support, Maintenance & Upgrades</li>
        </ul>
      </div>

      <hr className="border-slate-700" />

      {/* What Makes Us Different */}
      <div className="max-w-4xl mx-auto space-y-4">
        <h2 className="text-2xl md:text-3xl font-bold text-white text-center mb-2 tracking-tight">What Makes Us Different</h2>
        <ul className="grid md:grid-cols-2 gap-4 text-slate-300 text-base md:text-lg list-disc list-inside leading-relaxed">
          <li><strong>Fully Custom Builds</strong> – Every line of code matches your vision. No shortcuts.</li>
          <li><strong>Startup-Minded</strong> – We understand bootstrapping and moving fast.</li>
          <li><strong>Tech-Agnostic</strong> – We use the best tools for <em>your</em> product.</li>
          <li><strong>People-First Approach</strong> – Real humans, real conversations, real results.</li>
        </ul>
      </div>

      <hr className="border-slate-700" />

      {/* Team */}
      <div className="text-center max-w-3xl mx-auto space-y-4">
        <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Meet the Team</h2>
        <p className="text-slate-300 leading-relaxed">
          We’re a lean, skilled team of designers, developers, and product thinkers. Not a faceless agency—we’re hands-on builders who care about quality, timelines, and your success.
        </p>
        {/* Add team bios or photos here */}
      </div>

      <hr className="border-slate-700" />

      {/* Testimonials / Trust */}
      <div className="bg-slate-800 p-8 rounded-xl shadow-md max-w-4xl mx-auto space-y-6">
        <h2 className="text-2xl md:text-3xl font-bold text-center text-white tracking-tight">Trusted by Clients Around the World</h2>
        <div className="grid gap-6 md:grid-cols-2">
          <blockquote className="bg-slate-700 p-6 rounded-lg shadow-md italic text-slate-200">
            “TechMate built our MVP faster than we expected, and the results were beyond what we imagined.”<br />
            <span className="block mt-4 text-sm text-slate-400 font-medium">— Brian Harrington, Startup Founder</span>
          </blockquote>
          <blockquote className="bg-slate-700 p-6 rounded-lg shadow-md italic text-slate-200">
            “It’s rare to find a dev team that *gets it*—but TechMate really understood what we needed and delivered.”<br />
            <span className="block mt-4 text-sm text-slate-400 font-medium">— Brian Harrington, CEO of EliteX.com</span>
          </blockquote>
        </div>
        <div className="text-slate-400 text-center">
          ✅ 20+ successful projects · Clients in 5+ countries · 100% commitment to delivery
        </div>
      </div>

      {/* CTA */}
      <div className="text-center space-y-4">
        <h2 className="text-2xl md:text-3xl font-bold text-blue-400 tracking-tight">🚀 Ready to Build?</h2>
        <p className="text-slate-300 text-lg">Whether you're refining an idea or scaling a product, TechMate is ready to help.</p>
        <div className="flex justify-center gap-4">
          <a
            href="/quote"
            className="bg-blue-600 text-white px-6 py-3 rounded-full hover:bg-blue-700 hover:scale-105 focus:ring-2 focus:ring-blue-500 transition-transform duration-200"
          >
            Get a Quote
          </a>
          <a
            href="/contact"
            className="border border-blue-600 text-blue-400 px-6 py-3 rounded-full hover:bg-blue-900 hover:scale-105 focus:ring-2 focus:ring-blue-500 transition-transform duration-200"
          >
            Contact Us
          </a>
        </div>
      </div>
    </section>
  );
};

export default About;
