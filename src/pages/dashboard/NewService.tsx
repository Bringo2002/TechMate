...
const FONT_URL = 'https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;1,9..40,300&family=DM+Mono:wght@400;500&display=swap';

// Hoisted outside the component so the browser only parses the CSS once.
const NS_STYLES = `
  .ns-root {
...
const NewService = () => {
  ...
  // Inject Google Font <link> once — avoids render-blocking @import inside <style>
  useEffect(() => {
    const id = 'ns-dm-font';
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Guard against double-clicks — ref updates synchronously unlike state
    if (submittingRef.current) return;
    submittingRef.current = true;
    setLoading(true);

    try {
      // Snapshot form state before async work to avoid stale closure issues
      const payload = {
  ...
  const SelectedIcon = useMemo(
    () => ICONS.find(i => i.name === formData.iconName)?.icon || Briefcase,
    [formData.iconName]
  );
  const selectedColor = useMemo(
    () => COLORS.find(c => c.name === formData.color),
    [formData.color]
  );

  return (
    <>
      <style>{NS_STYLES}</style>
                    ...
                    {ICONS.map((i) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          title={i.name}
                          onClick={() => updateField('iconName', i.name)}
